...
impl NotificationInformation {
    pub fn new(effects: AccountAddress, amount: BigInt) -> Self {
        Self { effects, amount }
    }
}

fn convert<T: Into<BigInt>>(address: Address, amount: T, is_positive: bool) -> Option<NotificationInformation> {
...
fn get_cis2_events_addresses(effects: &AccountTransactionEffects) -> Vec<NotificationInformation> {
    match &effects {
        AccountTransactionEffects::AccountTransfer { to, amount } => vec![NotificationInformation::new(*to, amount.micro_ccd.into())],
        AccountTransactionEffects::AccountTransferWithMemo { to, amount, .. } => vec![NotificationInformation::new(*to, amount.micro_ccd.into())],
        AccountTransactionEffects::ContractUpdateIssued { effects } =>
            effects
                .iter()
                .flat_map(|effect| match effect {
                    ContractTraceElement::Transferred { to, amount,.. } => vec![NotificationInformation::new(*to, amount.micro_ccd.into())],
                ...
                .collect(),
        AccountTransactionEffects::TransferredWithSchedule { to, amount } => vec![NotificationInformation::new(*to, amount.iter().fold(BigInt::from(0), |acc, &(_, item)| acc + BigInt::from(item.micro_ccd)))],
        AccountTransactionEffects::TransferredWithScheduleAndMemo { to, amount, .. } => vec![NotificationInformation::new(*to, amount.iter().fold(BigInt::from(0), |acc, &(_, item)| acc + BigInt::from(item.micro_ccd)))],
        _ => vec![]
    }
}
